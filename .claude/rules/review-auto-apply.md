...
- PR author is a `speclynx` org member — verify with `gh api orgs/speclynx/members/<author_login>` (204 = member, 404 = not). Non-member PRs are review-only; use `review-community`, which never pushes to a contributor's branch. All findings on non-member PRs are comments only, regardless of category.
