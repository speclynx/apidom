...
const jsonSpec = fs
  .readFileSync(path.join(__dirname, 'fixtures', 'sample-overlay.json'))
  .toString();
const yamlSpec = fs
  .readFileSync(path.join(__dirname, 'fixtures', 'sample-overlay.yaml'))
  .toString();
...
describe('adapter', function () {
  context('given definition in YAML 1.2 format', function () {
    ...
    specify('should detect minor version bump', async function () {
      assert.isTrue(await adapter.detect('overlay: "1.2.0"'));
    });

    specify('should detect patch version bump', async function () {
      assert.isTrue(await adapter.detect('overlay: "1.0.1"'));
    });

    specify('should detect minor and patch version bump', async function () {
      assert.isTrue(await adapter.detect('overlay: "1.1.1"'));
    });
  });
  ...
  it('should parse', async function () {
    ...
    assert.isTrue(isOverlay1Element(parseResult.api));
  ...
  context('given YAML with empty node', function () {
    specify('should generate source maps', async function () {
      ...
      assert.equal(infoValue.startOffset, 20);
      assert.equal(infoValue.endLine, 1);
      assert.equal(infoValue.endCharacter, 5);
      assert.equal(infoValue.endOffset, 20);
    });
  });
  ...
  context('detectionRegExp', function () {
    ...
    specify('should reject invalid version ranges', function () {
      assert.isFalse(adapter.detectionRegExp.test('overlay: 1.01.0'));
      assert.isFalse(adapter.detectionRegExp.test('overlay: 1.0.x'));
      assert.isFalse(adapter.detectionRegExp.test('overlay: 3.1.0'));
    });
  });
});
