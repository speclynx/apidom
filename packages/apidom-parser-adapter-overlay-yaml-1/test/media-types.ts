...
import * as overlayYamlAdapter from '../src/adapter.ts';

describe('given adapter is used in parser', function () {
  const parser = new ApiDOMParser().use(overlayYamlAdapter);

  context('given Overlay 1.0.0 definition in YAML format', function () {
    specify('should find appropriate media type', async function () {
      const mediaType = await parser.findMediaType('overlay: "1.0.0"');

      assert.strictEqual(mediaType, 'application/vnd.oai.overlay+yaml;version=1.0.0');
    });
  });

  context('given Overlay 1.1.0 definition in YAML format', function () {
    specify('should find appropriate media type', async function () {
      const mediaType = await parser.findMediaType('overlay: "1.1.0"');

      assert.strictEqual(mediaType, 'application/vnd.oai.overlay+yaml;version=1.1.0');
    });
  });

  context('given Overlay 1.0.A definition in YAML format', function () {
  ...
  context('given Overlay future definition in YAML format', function () {
    specify('should not find appropriate media type', async function () {
