  ...
  context('strategies', function () {
    context('openapi-3-1', function () {
      context('Parameter Object', function () {
        context('given an external reference in the components/parameters field', function () {
          const fixturePath = path.join(rootFixturePath, 'external');
          const rootFilePath = path.join(fixturePath, 'root.json');

          specify(
            'should hoist the external fragment into components/parameters',
            async function () {
          ...
          specify('should rewrite the reference to an internal pointer', async function () {
            ...
            assert.strictEqual(
              toValue(evaluate(bundled.result as Element, '/components/parameters/Local/$ref')),
              '#/components/parameters/External',
            );
          });
