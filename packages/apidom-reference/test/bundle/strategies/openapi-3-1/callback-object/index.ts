  ...
  context('strategies', function () {
    context('openapi-3-1', function () {
      context('Callback Object', function () {
        context('given an external reference in the components/callbacks field', function () {
          const fixturePath = path.join(rootFixturePath, 'external');
          const rootFilePath = path.join(fixturePath, 'root.json');

          specify(
            'should hoist the external fragment into components/callbacks',
            async function () {
          ...
          specify('should rewrite the reference to an internal pointer', async function () {
            ...
            assert.strictEqual(
              toValue(evaluate(bundled.result as Element, '/components/callbacks/Local/$ref')),
              '#/components/callbacks/External',
            );
          });
