      ...
      context('Reference Object', function () {
        ...
        context('given external references colliding on name with different content', function () {
          ...
          specify(
            ...
            async function () {
              const bundled = await bundle(rootFilePath, {
                parse: { mediaType: mediaTypes.latest('json') },
                bundle: { strategyOpts: { 'openapi-3-1': { onComponentNameCollision: 'off' } } },
              });
        ...
        context('given an external reference with a dotted JSON Pointer key', function () {
          ...
          specify('should preserve the dotted key as the component name', async function () {
            ...
            assert.strictEqual(
              toValue(evaluate(bundled.result as Element, '/paths/~1a/get/parameters/0/$ref')),
              '#/components/parameters/my.org.User',
            );
          });
        });
        ...
        context('given nested external Reference Objects', function () {
          ...
          specify('should embed the nested external Schema resource', async function () {
            const bundled = await bundle(rootFilePath, {
              parse: { mediaType: mediaTypes.latest('json') },
            });

            // the nested $ref lives in a Schema Object, so it is embedded as a
            // JSON Schema compound-document resource (not hoisted/rewritten) — its
            // referencing $ref is preserved
            assert.strictEqual(
              toValue(
                evaluate(
                  bundled.result as Element,
                  '/components/responses/Pet/content/application~1json/schema/$ref',
                ),
              ),
              './b.json#/PetSchema',
            );
            assert.isDefined(toValue(evaluate(bundled.result as Element, '/components/schemas/b')));
          });
        });
        ...
        context('given circular external Reference Objects', function () {
          const fixturePath = path.join(rootFixturePath, 'external-circular-parameter');
          const rootFilePath = path.join(fixturePath, 'root.json');

          specify('should terminate and hoist both parameters once', async function () {
            const bundled = await bundle(rootFilePath, {
              parse: { mediaType: mediaTypes.latest('json') },
            });
          ...
          specify('should rewrite the cycle to internal pointers', async function () {
            ...
            assert.strictEqual(
              toValue(evaluate(bundled.result as Element, '/components/parameters/B/$ref')),
              '#/components/parameters/A',
            );
          });
        ...
        context('given an external reference whose pointer token contains a slash', function () {
          ...
          specify('should escape the slash in the internal pointer', async function () {
            ...
            assert.strictEqual(
              toValue(evaluate(bundled.result as Element, '/paths/~1a/get/parameters/0/$ref')),
              '#/components/parameters/Foo~1Bar',
            );
          });
        });
