  ...
  context('strategies', function () {
    context('openapi-3-0', function () {
      context('Security Scheme Object', function () {
        context('given an external reference in the components/securitySchemes field', function () {
          const fixturePath = path.join(rootFixturePath, 'external');
          const rootFilePath = path.join(fixturePath, 'root.json');

          specify(
            'should hoist the external fragment into components/securitySchemes',
            async function () {
              ...
              assert.property(
                toValue(
                  evaluate(bundled.result as Element, '/components/securitySchemes'),
                ) as object,
                'External',
              );
            },
          ...
          specify('should rewrite the reference to an internal pointer', async function () {
            ...
            assert.strictEqual(
              toValue(
                evaluate(bundled.result as Element, '/components/securitySchemes/Local/$ref'),
              ),
              '#/components/securitySchemes/External',
            );
          });
