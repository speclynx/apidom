  ...
  context('strategies', function () {
    context('openapi-3-0', function () {
      context('Request Body Object', function () {
        context('given an external reference in the components/requestBodies field', function () {
          const fixturePath = path.join(rootFixturePath, 'external');
          const rootFilePath = path.join(fixturePath, 'root.json');

          specify(
            'should hoist the external fragment into components/requestBodies',
            async function () {
          ...
          specify('should rewrite the reference to an internal pointer', async function () {
            ...
            assert.strictEqual(
              toValue(evaluate(bundled.result as Element, '/components/requestBodies/Local/$ref')),
              '#/components/requestBodies/External',
            );
          });
