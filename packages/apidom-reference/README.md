...
base URI (e.g. `schemas/ex.json`), so the bundle carries no absolute paths and stays self-contained when
moved as a directory tree. The referencing `$ref` is **rebased** onto the embedded
...
`components.inputs` carrying its `$id`. A resource without a `$id` of its own gets one assigned as a URI-reference relative
to the entry document's base URI — its `$self` when present — (e.g. `schemas/ex.json`), so the bundle carries
no absolute paths and stays self-contained when moved as a directory tree. The referencing
