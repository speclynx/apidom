...
`@speclynx/apidom-parser-adapter-arazzo-json-1` is a parser adapter for the **Arazzo 1.x.y specification** in [JSON format](https://www.json.org/json-en.html) supporting the following versions:
...
You can install `@speclynx/apidom-parser-adapter-arazzo-json-1` via [npm CLI](https://docs.npmjs.com/cli) by running the following command:
...
[Detection](https://github.com/speclynx/apidom/blob/main/packages/apidom-parser-adapter-arazzo-json-1/src/adapter.ts#L13) is based on a regular expression matching required Arazzo 1.x.y specification symbols in JSON format.
...
const parseResult = await parser.parse('{"arazzo": "1.1.0"}', { mediaType: arazzoJsonAdapter.mediaTypes.latest('json') });
```
